#!/usr/bin/env bash
# Test 02 — Root CA Certificate (Profile 9.1)
...
cert = issue_root_ca_certificate('Test-Root-CA', priv, pub)
...
from src.crypto import generate_keypair
from src.types import PublicKeyAlgorithm, IssuerChoice
from src.certificates import issue_root_ca_certificate
priv, pub = generate_keypair(PublicKeyAlgorithm.ECDSA_NIST_P256)
cert = issue_root_ca_certificate('Test-Root-CA', priv, pub)
...
from src.crypto import generate_keypair
from src.types import PublicKeyAlgorithm, IssuerChoice, CertificateType, CertIdChoice
from src.certificates import issue_root_ca_certificate
from src.encoding import decode_certificate
...
cert = issue_root_ca_certificate('Test-Root-CA-384', priv, pub, algorithm=PublicKeyAlgorithm.ECDSA_NIST_P384)
assert len(cert.encoded) > 50
